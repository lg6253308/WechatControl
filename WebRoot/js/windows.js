...
//判断预定是否登录
function DivWindowOpentishi(divWidth,divHeight,objDiv){
	//lockScreen();	//锁定背景
	divOpen(objDiv,divWidth,divHeight);	
     ...
     {  
       ...
       $("#username").css("border", "#7f9db9 1px solid");
       $("#password").css("border", "#7f9db9 1px solid");
       $("#passError").attr('class', '');
       $("#nameError").empty();
       $("#passError").empty();
       ...
       $("#"+objDiv).hide(200);
         ...
         $("#bank_tiaozhuang").click(function()
     {
       $(this).css("cursor","pointer");
       $("#divLock").remove();$("#"+objDiv).hide(200);
     });
     $("#nobank_tiaozhuang").click(function()
     {
       $(this).css("cursor","pointer");
       $("#divLock").remove();$("#"+objDiv).hide(200);
     });
...
function DivWindowOpenByDiv(divWidth,divHeight,objDiv){
	lockScreen();	//锁定背景
	divOpen(objDiv,divWidth,divHeight);	
	  $(".xbox-close").click(function()
     {
...
function DivWindowOpenadmin(divWidth,divHeight,objDiv){
	//lockScreen();	//锁定背景
	divOpen(objDiv,divWidth,divHeight);	
	$("#"+objDiv).css("background-color","#fff");
	$("#"+objDiv).css("border","solid 5px #999");
...
//CustomMade sdf 后台属性添加弹出框
function DivWindowOpenAttr(divWidth,divHeight,objDiv,valId,value){
    $("input[id*=txtName]").val(value);
    $("input[id*=txtName]").focus();
    $("input[id*=txtName]").blur();
    $("input[id*=valIdOnly]").val(valId);
...
function divOpen(objDiv,divWidth,divHeight){
		...
		$("#"+objDiv).css("border","solid 7px #b5e0f0");
		$("#"+objDiv).css("-moz-border-radius","5px 5px 5px 5px");

		
		
		
	//div移动			var _move=false;//移动标记
...
//					_move=true;
//					_x=e.pageX-parseInt($("#"+objDiv).css("left"));
//					_y=e.pageY-parseInt($("#"+objDiv).css("top"));
//					//$("#"+objDiv).css("cursor","move");
//					//$("#"+objDiv).fadeTo(0.01, 0.9);//点击后开始拖动并透明显示
//				});
//			
//			
//			
...
//						var y=e.pageY-_y;
//						$("#"+objDiv).css({top:y,left:x});//控件新位置
//					}
//				}).mouseup(function(){
//				_move=false;
//				//$("#"+objDiv).fadeTo("fast", 1);//松开鼠标后停止移动并恢复成不透明
//			});
//			});
