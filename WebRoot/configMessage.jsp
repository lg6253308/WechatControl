                ...
                <li style="position:relative">
	        		<form action="updateMessage" method="post" enctype="multipart/form-data">
		                ...
		                <div class="key_right">
			                ...
			                <a href="javascript:;" title="显示" class="save" onclick="saveMessage('${message.keyword}','${message.content}','${status.index}');"></a>
		                ...
		                <div id="edit${status.index}" class="formtitle" style="display:none; position:absolute;	top:35px; left:0; z-index:9999;" >
		                  ...
		                  <div class="firsttitle">
		                    ...
		                    <div class="selectframe" id="div1${status.index}" style="display:block">
		                    	<div class="firsttitle" style="background:#fff">消息标题：<input name ="title" type="text" value=${message.title} id="titletxt${status.index}" /></div>
		                    	<div style="width:250px;height:auto;margin:5px auto"><textarea name ="content" id="replytxt${status.index}" style="width:230px;border:1px solid #f0f0f0;min-height:80px">${message.content}</textarea></div>
		                    	<div class="saveframe"><input type="submit"  id="savebtnA" value="保存" class="savebtn" /></div>
		                    </div>
		                    <div class="selectframe" id="div2${status.index}">
		                    	<div style="width:260px;height:auto;margin:0 auto">
		                    	<div id="preview${status.index}">
		    						<img id="imghead${status.index}" border=0 src='${message.picture}' />
								</div>
								<input type="file" name="uploadImage" onchange="previewImage(this,${status.index})" /> 
		                    	</div>
		                    	<div style="width:250px;height:auto;margin:5px auto" value="图文描述">
                    			<textarea id="replytxt1${status.index}" name ="content" style="width:230px;border:1px solid #f0f0f0;min-height:80px">${message.content}</textarea></div>
                    			<div class="firsttitle">图文链接：<input type="text" value="" name ="linkUrl" id="urltxt${status.index}" /></div>
...
//-->
		...
		function saveMessage(keyword,content,index){
			oPhone.style.display = 'block';
			imgPhone.style.display = 'none';
			var oPre = document.getElementById('preview'+index);
			var oTitle = document.getElementById('titletxt'+index);
						oPhone.style.display = 'block';
						imgPhone.style.display = 'none';
						oAsktitle.innerHTML = keyword;
						 if('\">'==oPre.innerHTML.split("src=\"")[1].trim()){
					       oMyinfo.innerHTML = document.getElementById('replytxt'+index).value;
					  	
					   		 }else{
					   		   oMyinfo.innerHTML = oTitle.value+
					   		   '<br>' +oPre.innerHTML.replace('imghead'+index+'\"','imgheadS\" style=\"width:130px;height:100px;\"')
					   		   +'<br>'+document.getElementById('replytxt1'+index).value ;
					    	}
		}
		/*
